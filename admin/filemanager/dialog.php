...
$tmp = get_absolute_path($tmp_dir . '/' . input('file'));
if($tmp === false)
    err(404,'File or Directory Not Found');
if(substr($tmp, 0,strlen($tmp_dir)) !== $tmp_dir)
    err(403,"Forbidden");
if(strpos(input('file'), DIRECTORY_SEPARATOR) === 0)
    err(403,"Forbidden");
...
if(input_get('do') == 'list') {
    ...
    exit;
} elseif (input_post('do') == 'delete') {
    if($allow_delete) {
        rmrf($file);
    }
    exit;
} elseif (input_post('do') == 'mkdir' && $allow_create_folder== true) {
    ...
    @mkdir($_POST['name']);
    exit;
} elseif (input_post('do') == 'upload' && $allow_upload == true) {
    var_dump($_POST);
    var_dump($_FILES);
...
<html><head>
    ...
    <script src="<?php echo page()->assets_admin; ?>/js/jquery.min.js"></script>
    <script>
        (function($){
        ...
        function select_object (url) {
            // If url is empty then exit
            if (!url) return false;
            var
                temp_array = window.location.search.substring(1).split('&'),
                params = {}
            ;
            // Get window search arguments as object
            for (var i = temp_array.length; i-- > 0;) {
                var tmp = temp_array[i].split('=');
                params[tmp[0]] = tmp[1];
            }
