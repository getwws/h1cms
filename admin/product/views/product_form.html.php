            ...
            <div class="ibox-body">
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane p-10" id="tab-general" role="tabpanel">
                        ...
                        <div class="form-group row">
                            <label class="col-lg-2 col-form-label rigth-label" for="node-node-status-1">产品状态</label>
                        ...
                        <div class="table-responsive">
                            <table id="images" class="table table-striped table-bordered table-hover">
                                ...
                                <?php foreach($product_images as $i=>$image):
                                    $thumb_img = Product::createThumb($image->image,100,100);
                                    ?>
                                <tr id="productimage-row<?php echo $i;?>">
                                    <td class="text-left">
                                        <a href="#" data-target-id="input-img<?php echo $i;?>" data-thumb="thumb-img<?php echo $i;?>"
                                           data-toggle="modal" data-target="#image_filemangent" >
                                            <img src="<?php echo $thumb_img;?>" id="thumb-img<?php echo $i;?>" class="img-thumbnail"></a>
                                        <input type="hidden" name="product_image[<?php echo $i;?>][image]" value="<?php echo $image->image;?>" id="input-img<?php echo $i;?>"></td>
                                    <td class="text-right"><input type="text" name="product_image[<?php echo $i;?>][sort_order]" value="<?php echo $image->sort_order;?>" placeholder="Sort Order" class="form-control"></td>
                                    <td class="text-left"><button type="button" onclick="$('#productimage-row<?php echo $i;?>').remove();" data-toggle="tooltip" title="" class="btn btn-danger" data-original-title="Remove"><i class="fa fa-minus-circle"></i></button></td>
                                </tr>
                                <?php endforeach;?>
                                ...
                                <tr>
                                    ...
                                    <td colspan="3" class="text-center">
                                        ...
                                        <button class="btn btn-default" type="button" data-toggle="modal" data-target="#image_filemangent" data-selected="product_image_callback"><i class="fa fa-search"></i> 从库中选择文件</button>
...
<script>
    function product_image_callback(path,thumb){
        addImage(path,thumb);
        $('#image_filemangent').modal('hide');
    }
    $(function () {
        //editor
        APP.image_manager();
        var myDropzone = new Dropzone("#upload_file", {
            ...
            successmultiple:function(file,response) {
                var image_list = jQuery.parseJSON(response);
                $.each(image_list,function(index,item){
                   addImage(item.image,item.thumb);
                });
            },
    ...
    var product_image_row = <?php echo count($product_images);?>;
    function addImage(path,thumb){
        html  = '<tr id="product-image-row' + product_image_row + '">';
        html += '  <td class="text-left"><a href="#"  data-target-id="input-image' + product_image_row + '" data-thumb="thumb-image' + product_image_row + '" data-toggle="modal" data-target="#image_filemangent">'
        html += '<img src="'+thumb+'" id="thumb-image' + product_image_row + '" class="img-thumbnail" /></a><input type="hidden" name="product_image[' + product_image_row + '][image]" value="'+path+'" id="input-image' + product_image_row + '" /></td>';
        html += '  <td class="text-right"><input type="text" name="product_image[' + product_image_row + '][sort_order]" value="0" placeholder="Sort Order" class="form-control" /></td>';
