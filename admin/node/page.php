...
{
    page()->setTitle('单页管理');
    page()->setLeftMenuActive('node.page');

    $st = input_get('st', 'title');
    $s = input_get('s', '');
    $query = DB::select('node AS n,node_language as nl','count(n.node_id) as rowcount');
    ...
    render('node.page', ['nodes' => $nodes, 'pagehelper' => $pagehelper]);
}
...
function editGET()
{
    ...
    render('node.page_form', ['categories' => $c->getList(), 'node' => $node, 'language' => $language, 'relationships' => $relationships]);
}
...
function editPOST()
{
    ...
    $data_node['updated_at'] = time();
    $model = new Node();
    $result = $model->updateNode($node_id, $data_node, $data_languages, 0);
    if ($result) {
        add_flash('文章添加成功', SESSION_SUCCESS);
        redirect(url_for('/node/page.php'));
    } else {
